...
% Zielgruppe gehört im Prinzip zu dieser Section, genauso wie Anwendungsbereich
...
\item Zugriff auf Kontakten ist wünschenswert für bessere User-Experience. %inwiefern? Machen wir das nicht über eine Datenbank
\end{itemize}
...
\begin{enumerate}[label={\textbf{/F\protect\threedigits{\theenumi}0/}}, leftmargin=*, resume]	
	...
	\begin{itemize}
		...
		\item Clustering-Schwellwert (vgl. \ref{Clustering})
	\end{itemize}
...
\begin{enumerate}[label={\textbf{/F\protect\threedigits{\theenumi}0/}}, leftmargin=*, resume]	
	...
	\begin{itemize}
		...
		\item Clustering-Schwellwert (vgl. \ref{Clustering})
	\end{itemize}
	ändern. Der neue Startzeitpunkt darf nicht in der Vergangenheit liegen. Sobald die Änderungen vom Benutzer bestätigt wurden, werden sie vom System übernommen und den Mitgliedern des GOs angezeigt, wenn diese die GO-Detailansicht aufrufen.
...
\begin{enumerate}[label={\textbf{/F\protect\threedigits{\theenumi}0/}}, leftmargin=*, resume]	
	...
	\item \textit{Anonymisierung der Benutzer}\label{Anonymisierung} \\ Die Funktionalität des Systems soll die Anonymität des Benutzers bestmöglich sicherstellen. Dazu sind folgende Funktionen der App zu implementieren:
		\begin{itemize}
			...
			\item Die Genauigkeit der Standortbestimmung kann durch den Clustering-Schwellwert von dem GO-Verantwortlichen selbst bestimmt werden.
			\item Bei GOs, bei denen weniger als 5 Benutzer die Standortverfolgung aktiviert haben, erhält der Benutzer beim Losgehen eine Warnung: er kann den Start der Standortverfolgung solange verzögern, bis mindestens 4 andere Benutzer ebenfalls losgegangen sind. 
			...
			\item Benutzer haben zu jeder Zeit die Möglichkeit die Standortverfolgung zu beenden, indem sie ihren Teilnahmestatus auf 'Abgeleht' setzen.
		\end{itemize}
	...
	\item \textit{Benachrichtigungen senden}\\
...
\begin{enumerate}[label={\textbf{/F\protect\threedigits{\theenumi}0/}}, leftmargin=*, resume]	
	...
	Der Benutzer kann entscheiden, ob er Android-Benachrichtigungen von der App erhalten will oder nicht. Er kann dies selbst über einen Slider in der Einstellungs-Ansicht einstellen (vgl. *GUI-Ansicht*).
...
\newpage
\section{Use cases und globale Testszenarien}

\subsection{Use Case: Erstellung und Verwaltung eines Benutzeraccounts}
Dieser Use Case beschreibt die Interaktion eines Benutzers (hier: Bob) mit dem System zum Einen mit dem Ziel, einen Benutzeraccount zu erstellen und die Benutzerdaten anzupassen, zum Anderen mit dem Ziel den Benutzeraccount wieder zu löschen.

\textbf{Akteure:} Bob (Benutzer) \\

\includegraphics[width=1\textwidth]{Use_Cases/use_case_Benutzerprofil.png}

\subsubsection{Szenario 1}
Bob will die App GO nutzen. Dazu erstellt er einen Benutzeraccount, fügt persönliche Daten seinem Benutzerprofil hinzu und loggt sich anschließend wieder aus.\\

\textbf{Testfälle:}
\begin{enumerate}[label={\textbf{/T\protect\threedigits{\theenumi}0/}}, leftmargin=*]
	\item\label{Registrieren-Test} Bob öffnet die App, gibt die Anmeldedaten seines gültigen Google-Accounts ein (E-Mailadresse: bob-testperson@gmail.com, Passwort: passwort) und klickt auf den Button 'Sign In'.
	\item Bob ist in seinem Benutzeraccount angemeldet. Bob öffnet seine Profilansicht und klickt auf den Button 'Profil ändern'. Er gibt in das Textfeld 'Benutzername' den Namen 'Bob' ein. Er klickt auf den Button 'Bild wählen'. Er wählt ein Bild aus dem Foto-Ornder des Android-Geräts aus. Er klickt auf den Button 'Änderungen speichern'.
...
Bob will die App GO nicht mehr benutzen. Er will deshalb seinen Benutzeraccount löschen. \\

\textbf{Testfälle:}
\begin{enumerate}[label={\textbf{/T\protect\threedigits{\theenumi}0/}}, leftmargin=*, resume]
	\item Bob meldet sich in seinem Benutzeraccount an. Er benutzt dieselben Login-Daten wie in \ref{Registrieren-Test}. 
	\item Bob ist in seinem BEnutzeraccount angemeldet. Er öffnet seine Profilansicht und klickt auf den Button 'Profil löschen'.
\end{enumerate}

\subsection{Use Case: Erstellung und Verwaltung einer neuen Gruppe}
Dieser Use Case beschreibt die Interaktion mehrerer Benutzer mit dem System mit dem Ziel, gemeinsam eine Gruppe zu gründen bzw. eine gemeinsame Gruppe zu löschen.\\

\textbf{Akteure:} Bob (Administrator), Alice (Gruppenmitglied), Peter (Benutzer) \\

\includegraphics[width=1\textwidth]{Use_Cases/use_case_Gruppen.png}

\subsubsection{Szenario 3}
Bob, Alice und Peter wollen gemeinsam eine Gruppe erstellen, um ihre Treffen organisieren zu können. Bob soll der Administrator dieser Gruppe sein.\\
...
\begin{enumerate}[label={\textbf{/T\protect\threedigits{\theenumi}0/}}, leftmargin=*, resume]
	...
	\item Bob öffnet die Gruppendetailansicht und klickt auf das Feld 'Mitglied hinzufügen'. Er gibt in das Suchfeld 'alice-testperson@gmail.com' ein. Er wählt aus den Suchresultaten Alice's Profil aus.
...
Bob tritt aus der Gruppe 'Bar' wieder aus. Da Alice nun das einzige Mitglied ist, entschließt sie sich die Gruppe wieder zu löschen. \\
...
\subsection{Use Case: Erstellung und Ausführung eines GOs}
Dieser Use Case beschreibt die Interaktion mehrerer Benutzer, die gemeinsam in einer Gruppe sind, mit dem System, mit dem Ziel ein GO zu erstellen und auszuführen.\\

\textbf{Akteure:} Bob (GO-Verantwortlicher), Alice (Gruppenmitglied)\\

\includegraphics[width=1\textwidth]{Use_Cases/use_case_GO.png}

\subsubsection{Szenario 5}
Bob erstellt ein neues GO in der Gruppe 'Bar'. Die Gruppe hat mittlerweile 3 neue Mitglieder (Gesamtmitgliederzahl: 5). Alice tritt dem GO bei und bestätigt 10min vor dem Startzeitpunkt, dass sie losgegangen ist. Alice entfernt sich während des GOs von der Gruppe und will nicht, dass man ihren Standort verfolgen kann. Sie verlässt daher das GO.\\

\textbf{Testfälle:}
\begin{enumerate}[label={\textbf{/T\protect\threedigits{\theenumi}0/}}, leftmargin=*, resume]
	\item Bob erstellt ein GO mit folgenden Daten:
	\begin{itemize}
	...
	\item Alice ändert ihren Teilanhmestatus auf 'Abgelehnt'. Ihr Standort ist für die anderen nicht mehr sichtbar.
	\item das GO endet zum Endzeitpunkt automatisch.
