...
\newglossaryentry{GO-Verantwortlicher}
{
	name={GO-Verantwortlicher},
	description={Der Ersteller eines GOs wird immer zum GO-Verantwortlichen desselben. Er ist verantwortlich für die Adminsitration des GOs (Daten ggfs. ändern, GO vorzeitig beenden, etc.). Ihm stehen mehr Funktionen zur Verfügung wie normalen GO-Teilnehmern},
}
\newglossaryentry{Administrator}
{
	name={Administrator},
	description={Der Administrator ist für die Verwaltung einer Gruppe zuständig. Dazu gehört beispielsweise das Hinzufügen und Entfernen von Gruppenmitgliedern oder das Einpflegen von Gruppendaten. Jede Gruppe muss stets mindestens einen Adminsitrator haben. Es ist zulässig, dass eine Gruppe mehr als einen Administrator hat}
}
...
\begin{itemize}
\item Es werden weder direkte Messagingfunktionen, noch andere ähnliche Funktionen von Social Media Plattformen wie Facebook (Statusmeldungen, Kommentare, etc.) geboten.
...
Die GO! App ist eine mobile Applikation, die speziell zur Organisation von Treffen (z. B. gemeinsames Essen im Café oder in der Mensa) entwickelt wird. Bei dem erfolgreichen gemeinsamen Losgehen wird der gemittelte GPS-Standort von Mitgliedern der Gruppe angezeigt.
...
\begin{enumerate}[label={\textbf{/NF\protect\threedigits{\theenumi}0/}}, leftmargin=*]
		...
		Um den Datenschutz, beziehungsweise die Privatsphäre der Benutzer zu garantieren, findet die Lokalisierung und Mittelung der Standorte (via GPS) der Mitglieder einer Gruppe, die sich einem Event angeschlossen haben, erst ab einer Teilnehmerzahl von 3 statt. 
		...
		Die Aktualisierung der Standortmittelung findet alle 8 Sekunden statt.
		...
		Für sämtliche Funktionen gilt: Um von der Hauptansicht der App (nach der Anmeldung) zur Ausführungsaktivität zu gelangen sind maximal 3 Klicks nötig. Die Ausführungsaktivität ist die Ansicht der App, in der die Ausführung der Funktion beginnt (z.B. beginnt die Ausführung der Funktion 'Gruppenmitglied hinzufügen', nachdem er Button 'Mitglied hinzufügen' in der Gruppenänderungsansicht gedrückt wurde).
...
\textbf{Hoher Fokus:}\\ Sowohl die Bedienbarkeit als auch die Verständlichkeit der App erhalten bei der Entwicklung einen großen Fokus. Wir legen viel Wert auf ein simples und verständliches Design, um eine möglichst einfache und unkomplizierte Bedienung der App zu erlauben. Die volle Ausnutzen der Funktionalitäten soll im Mittelpunkt stehen. %Ich hab den einen Satz mal in 3 Teile geteilt :) war sonst ein bisschen lang
