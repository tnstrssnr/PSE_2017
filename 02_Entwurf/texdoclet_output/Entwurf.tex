...
\entityintro{Observable}{edu.kit.pse17.go_app.ServiceLayer.Observable}{Dieses Interface ist Teil einer Implemtierung eines Beobachter-Entwurfsmusters.}
\vskip .13in
\hbox{{\bf  Classes}}
...
Dieses Interface ist Teil einer Implemtierung eines Beobachter-Entwurfsmusters. Es übernimmt die Rolle des abstrakten Subjekts. Es muss von allen Klassen, die beobachtet werden müssen implementiert werden. In dieser Anwendung sind dies die DAO Klassen. Diese werden beobachtet, um Änderungen am Datenbestand zu bemerken und diese Änderungen an betroffene Clients weiterleiten zu können. Das Generic T gibt an, welcher Datentyp von den Änderungen betroffen ist und von den Beobachtern an die Clients weitergeleitet weden muss.\vskip .1in 
\subsubsection{Declaration}{
\begin{lstlisting}[frame=none]
...
\subsubsection{Method summary}{
\begin{verse}
\hyperlink{edu.kit.pse17.go_app.ServiceLayer.Observable.notify(edu.kit.pse17.go_app.ServiceLayer.Observable, T)}{{\bf notify(Observable, T)}} \\
...
\begin{itemize}
\item{ 
\index{notify(Observable, T)}
\hypertarget{edu.kit.pse17.go_app.ServiceLayer.Observable.notify(edu.kit.pse17.go_app.ServiceLayer.Observable, T)}{{\bf  notify}\\}
\begin{lstlisting}[frame=none]
void notify(Observable observable,java.lang.Object t)\end{lstlisting} %end signature
\begin{itemize}
\item{
{\bf  Parameters}
  \begin{itemize}
...
\subsubsection{Method summary}{
\begin{verse}
\hyperlink{edu.kit.pse17.go_app.ServiceLayer.Observer.update(edu.kit.pse17.go_app.ClientCommunication.Downstream.EventArg)}{{\bf update(EventArg)}} \\
\end{verse}
}
\subsubsection{Constructors}{
\vskip -2em
...
\hypertarget{edu.kit.pse17.go_app.ServiceLayer.Observer.update(edu.kit.pse17.go_app.ClientCommunication.Downstream.EventArg)}{{\bf  update}\\}
\begin{lstlisting}[frame=none]
public abstract void update(edu.kit.pse17.go_app.ClientCommunication.Downstream.EventArg arg)\end{lstlisting} %end signature
}%end item
\end{itemize}
}
}
